...
function sanitize_for_amp($content){
    ...
    $content = preg_replace('/<iframe width="(.*?)" height="(.*?)" src="https:\/\/www.youtube.com\/embed\/(.*?)" frameborder="0" allowfullscreen><\/iframe>.*<\/div>/i','<div class=embed-container><amp-youtube width="$1" height="$2" layout="responsive" data-videoid="$3"></amp-youtube></div>',$content,-1,$youtube_2_count);
    ...
    $content = str_replace('<script async src="//cdn.embedly.com/widgets/platform.js"></script>','',$content);
