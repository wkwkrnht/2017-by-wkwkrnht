...
function sanitize_for_amp($content){
    ...
    $content = preg_replace('/<blockquote class="twitter-tweet"(.*?)>(.*?)<a href="https:\/\/twitter.com\/(.*?)\/status\/(.*?)"(.*?)<\/blockquote>(.*?)<script async src="\/\/platform.twitter.com\/widgets.js" charset="utf-8"><\/script>/i','<div class=embed-container><amp-twitter width="800" height="600" layout="responsive" data-tweetid="$4" data-conversation="all" data-align="center"></amp-twitter></div>',$content,-1,$tw_count);
